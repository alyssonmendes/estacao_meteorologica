...
template<class T>
T Fila<T>::readFirst()  //Envia para a serial no formato
{
   return head->getVal();
...
template<class T>
string Fila<T>::enviaFirst()  //Envia para a serial no formato
{
...
template<class T>
void Fila<T>::removeAll(){
    int i = 0;
    int tam = tamanhoFila();
    for(i = 0; i < tam; i++)
        if(head != 0)
            removeFirst();
    cout << i << " registros excluidas!\n" ;
}
...
void Fila<T>::listAll()
{
    ...
    do
    {
        ...
        cout << fixed << std::setprecision(3) << aux->getVal()  << " oC\t|"<< endl  ;
